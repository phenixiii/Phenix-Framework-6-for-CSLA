...
namespace Phenix.StandardService.Test
{
  class Program
  {
    static void Main(string[] args)
    {
      Console.WriteLine("需事先在本机启动Host服务（启动Bin目录下的Phenix.Services.Host.x86/x64.exe程序，并注册过Phenix.StandardService.Plugin.dll）");
      ...
      while (true)
        try
        {
          ...
          if (LogOnHelper.TryLogOn("127.0.0.1", "ADMIN", password, out context))
            break;
          Console.WriteLine("登录失败! ：" + context.Message);
      ...
      using (Phenix.StandardService.Business.SynchronizeClocker clocker = Phenix.StandardService.Business.SynchronizeClocker.Run())
      {
        for (int i = 0; i < 100; i++)
        {
