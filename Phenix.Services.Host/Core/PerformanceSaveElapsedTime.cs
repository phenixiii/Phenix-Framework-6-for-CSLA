...
namespace Phenix.Services.Host.Core
{
  ...
  /// Save性能耗时
  /// </summary>
  internal class PerformanceSaveElapsedTime
  {
    public PerformanceSaveElapsedTime(string businessName)
    {
      _businessName = businessName;
      _maxElapsedTime = new PerformanceSaveMaxElapsedTime(businessName, 0, 0, null);
    }
    ...
    //最大耗时(秒)
    private PerformanceSaveMaxElapsedTime _maxElapsedTime;
    ...
    /// 检查最大耗时
    /// </summary>
    public bool CheckMaxElapsedTime(PerformanceSaveMaxElapsedTime newMaxElapsedTime)
    {
      try
      {
        _tally = _tally + 1;
      ...
      finally
      {
        if (_tally < 100 && _tally % 10 == 5 || _tally % 100 == 0)
          try
          {
            string directory = Path.Combine(AppConfig.BaseDirectory, "PerformanceSave");
