...
namespace Phenix.Test.使用指南._11._3._2._1._4
{
  ...
  {
    //* 演示：如果将Detail、Link对象自行做缓存（赋值为字段），必须标记[Csla.NotUndoable]
    [Csla.NotUndoable]
    private User _linkUser_DemoOk;
    /// <summary>
    /// 关联的用户
    /// </summary>
    public User LinkUserDemoOk
    {
      get
      {
        if (_linkUser_DemoOk == null)
          _linkUser_DemoOk = GetLink<User>();
        return _linkUser_DemoOk;
      }
    }

    //* 演示：如果将Detail、Link对象自行做缓存（赋值为字段），未标记[Csla.NotUndoable]会对回滚机制造成什么影响
    //[Csla.NotUndoable]
    private User _linkUser_DemoError;
    /// <summary>
    /// 关联的用户
    /// </summary>
    public User LinkUserDemoError
    {
      get
      {
        if (_linkUser_DemoError == null)
          _linkUser_DemoError = GetLink<User>();
        return _linkUser_DemoError;
      }
    }

    //* 一般情况下，获取Link对象的属性就这么写
    /// <summary>
    /// 关联的用户
    /// </summary>
    public User LinkUser
    {
      get { return GetLink<User>(); }
    }
