...
namespace Phenix.Test.使用指南._12._6._2._12
{
  ...
  {
    //* 在服务端添加限制保存的条件
    protected override void OnUpdatingSelf(DbTransaction transaction, ref List<CriteriaExpression> limitingCriteriaExpressions)
    {
      base.OnUpdatingSelf(transaction, ref limitingCriteriaExpressions);
      //* AdminUserNumber不允许被锁
      if (Usernumber == Phenix.Core.Security.UserIdentity.AdminUserNumber)
        if (Locked.HasValue && Locked.Value)
        {
          if (limitingCriteriaExpressions == null)
            limitingCriteriaExpressions = new List<CriteriaExpression>();
          limitingCriteriaExpressions.Add(UsernumberProperty == Phenix.Core.Security.UserIdentity.AdminUserNumber);
        }
    }
  }
