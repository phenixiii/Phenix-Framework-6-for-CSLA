...
namespace Phenix.Test.使用指南._17._3
{
  ...
  public class User : User<User>
  {
    ...
    {
      get { return GetCompositionDetail<UserRoleList, UserRole>(p => p.Inputer == ""); }
    }
  }
  ...
  public abstract class User<T> : Phenix.Business.BusinessBase<T> where T : User<T>
  {
    ...
    /// New
    /// </summary>
    public static T New(string usernumber, string password, DateTime? passwordchangedtime, string name, DateTime? login, DateTime? logout, DateTime? loginfailure, int? loginfailurecount, string loginaddress, long? US_DP_ID, long? US_PT_ID, int? locked)
    {
      T result = NewPure();
